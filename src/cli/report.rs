    ...
    for (file, mut group) in by_file {
        ...
        for f in group {
            ...
            if let Some(range) = f.line_range {
                writeln!(out, " {}", a.dim(&format!("at {file}:{}", range.start)))?;
            }
...
fn write_text_summary(out: &mut impl Write, summary: &ScanSummary, a: &Ansi) -> io::Result<()> {
    ...
    if a.use_color {
        ...
        if low > 0 {
            write!(out, "{}{}LOW {low}{} ", Ansi::BOLD, Ansi::CYAN, Ansi::RESET)?;
        }
        writeln!(
            out,
            "{} AI likelihood {}{}{pct:.0}%{}",
...
///
/// by VS Code (SARIF Viewer), GitHub Code Scanning, and other SARIF-aware
...
mod tests {
    ...
    #[test]
    fn sarif_report_is_valid_sarif_json() {
        let findings = vec![make_finding(Severity::High, "src/main.rs")];
        let summary = build_summary(&findings);
        let mut out = Vec::new();
        write_sarif_report(&mut out, &findings, &summary).expect("write");
        let text = String::from_utf8(out).expect("utf8");
        let parsed: serde_json::Value = serde_json::from_str(&text).expect("valid JSON");
        assert_eq!(parsed.pointer("/version").expect("version"), "2.1.0");
        assert!(
            parsed
                .pointer("/$schema")
    ...
    #[test]
    fn sarif_report_contains_results_and_rules() {
        let findings = vec![
            make_finding(Severity::High, "src/main.rs"),
            make_finding(Severity::Low, "src/lib.rs"),
        ];
    ...
    #[test]
    fn sarif_severity_maps_correctly() {
        let findings = vec![
            make_finding(Severity::High, "a.rs"),
            make_finding(Severity::Medium, "b.rs"),
            make_finding(Severity::Low, "c.rs"),
        ];
        let summary = build_summary(&findings);
        let mut out = Vec::new();
        write_sarif_report(&mut out, &findings, &summary).expect("write");
        ...
        let parsed: serde_json::Value = serde_json::from_str(&text).expect("valid JSON");
        assert_eq!(
            parsed.pointer("/runs/0/results/0/level").expect("level 0"),
            "error"
        );
        assert_eq!(
            parsed.pointer("/runs/0/results/1/level").expect("level 1"),
            "warning"
        );
        assert_eq!(
            parsed.pointer("/runs/0/results/2/level").expect("level 2"),
            "note"
        );
    }

    #[test]
    fn sarif_includes_line_range_when_present() {
        let mut f = make_finding(Severity::High, "src/main.rs");
        f.line_range = Some(crate::detection::finding::LineRange::new(10, 20).expect("range"));
        let summary = build_summary(std::slice::from_ref(&f));
        let mut out = Vec::new();
        write_sarif_report(&mut out, &[f], &summary).expect("write");
        let text = String::from_utf8(out).expect("utf8");
        let parsed: serde_json::Value = serde_json::from_str(&text).expect("valid JSON");
        let region_path = "/runs/0/results/0/locations/0/physicalLocation/region";
        assert_eq!(
            parsed
                .pointer(&format!("{region_path}/startLine"))
                .expect("startLine"),
            10
        );
        assert_eq!(
            parsed
                .pointer(&format!("{region_path}/endLine"))
                .expect("endLine"),
            20
        );
    }
    ...
    fn sarif_empty_findings_produces_empty_results() {
        let summary = build_summary(&[]);
        let mut out = Vec::new();
        write_sarif_report(&mut out, &[], &summary).expect("write");
        let text = String::from_utf8(out).expect("utf8");
        let parsed: serde_json::Value = serde_json::from_str(&text).expect("valid JSON");
        let results = parsed
            .pointer("/runs/0/results")
    ...
    #[test]
    fn sarif_includes_suggestion_as_fix() {
        let mut f = make_finding(Severity::Medium, "src/lib.rs");
        f.suggestion = Some("use a different word".to_owned());
        let summary = build_summary(std::slice::from_ref(&f));
        let mut out = Vec::new();
        write_sarif_report(&mut out, &[f], &summary).expect("write");
        let text = String::from_utf8(out).expect("utf8");
        ...
        assert!(
            parsed
                .pointer("/runs/0/results/0/fixes")
                .is_some_and(serde_json::Value::is_array)
        );
        assert!(
            parsed
                .pointer("/runs/0/results/0/fixes/0/description/text")
                .and_then(serde_json::Value::as_str)
                .is_some_and(|s| s.contains("different word"))
        );
    }

    #[test]
    fn sarif_includes_papertowel_summary_properties() {
        let findings = vec![make_finding(Severity::High, "a.rs")];
        let summary = build_summary(&findings);
        let mut out = Vec::new();
        write_sarif_report(&mut out, &findings, &summary).expect("write");
        let text = String::from_utf8(out).expect("utf8");
        let parsed: serde_json::Value = serde_json::from_str(&text).expect("valid JSON");
        let base = "/runs/0/properties/papertowel";
        assert_eq!(
            parsed
                .pointer(&format!("{base}/totalFindings"))
                .expect("totalFindings"),
            1
        );
        assert!(
            parsed
                .pointer(&format!("{base}/aiProbability"))

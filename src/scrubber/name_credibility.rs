...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        fs::write(temp.path().join("src/main.rs"), "fn main() {}\n")?;
        // Package name: "ai-tool-app" → "ai"(1), "tool"(2), "app"(3) = 3 generic hits ✓
        fs::write(
            temp.path().join("Cargo.toml"),
            "[package]\nname = \"ai-tool-app\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
        )?;
        // README mentions "ai-tool-app" 6 times → repetition_hits = 6 >= 5 ✓
        fs::write(
            temp.path().join("README.md"),
    ...
    fn missing_cargo_toml_name_returns_error() -> Result<(), Box<dyn std::error::Error>> {
        // Covers line 124: Cargo.toml present but missing [package] name → Validation error.
        use std::fs;
        use tempfile::TempDir;
        let temp = TempDir::new()?;
        fs::create_dir_all(temp.path().join("src"))?;
    ...
    fn repo_without_readme_produces_zero_repetitions() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let temp = TempDir::new()?;
        fs::create_dir_all(temp.path().join("src"))?;
        // Write many code files and a generic Cargo.toml name to trigger the finding path
        for i in 0..4_u8 {
            fs::write(
    ...
    fn git_directory_and_non_file_entries_are_skipped() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Create a subdirectory (triggers !path.is_file() skip at line 169).
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        // Covers lines 78 (Severity::Medium), 85 (mul_add .4), 100 (format! content).
        // Medium path: NOT (generic_hits >= 3 && repetition_hits >= 5 && code_files <= 2).
        // Use generic_hits = 2 (< 3) with enough repetitions/thin impl to trigger finding.
        use std::fs;
        use tempfile::TempDir;
        let temp = TempDir::new()?;
        fs::create_dir_all(temp.path().join("src"))?;
        // 1 code file → thin_implementation = true (code_files <= max_code_files_for_flag=4)
        fs::write(temp.path().join("src/main.rs"), "fn main() {}\n")?;
        // "app-tool" → "app"(1 generic hit), "tool"(2nd hit) but NOT >=3 → Medium
        fs::write(
            temp.path().join("Cargo.toml"),
            "[package]\nname = \"app-tool\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
        )?;
        // README mentions "app-tool" 4 times (>= min_name_repetition_hits=3) → noisy_discovery=true
        fs::write(
            temp.path().join("README.md"),

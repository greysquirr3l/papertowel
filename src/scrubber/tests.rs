...
pub fn detect_in_text(
	...
	let confidence = (metrics.dominant_prefix_ratio * 0.6 + metrics.assert_density * 0.4).min(1.0);
...
fn analyze_test_shape(content: &str) -> Result<TestShapeMetrics, PapertowelError> {
	...
	for line in content.lines() {
		...
		if trimmed.contains("assert_") || trimmed.starts_with("assert!(") || trimmed.starts_with("assert_eq!(") {
			assert_count += 1;
		}
...
#[cfg(test)]
mod tests {
	use crate::scrubber::tests::{
		DETECTOR_NAME, TestShapeDetectionConfig, detect_in_text,
	};
	...
	#[test]
	fn test_shape_detector_ignores_varied_tests() {
		let content = "\
...
";
		...
		let findings = match findings {
			...
			Err(error) => panic!("unexpected tests detector error: {error}"),
		};
		assert!(findings.is_empty());
	}

	#[test]
	fn test_shape_detector_flags_repeated_template_style() {
		let content = "\

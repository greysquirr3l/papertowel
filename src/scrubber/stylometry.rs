...
//! dimensions:
//!
//! * Sentence-length burstiness - coefficient of variation in word
//!   counts across sentences; LLMs produce uniform lengths < 0.35;
//!   humans vary > 0.55.
//! * Lexical diversity - MATTR-50, moving-average type-token ratio
//!   over 50-word sliding windows; LLMs cluster 0.68 to 0.76.
//! * Weighted AI phrase n-gram density - counts occurrences of 24
...
//! ## Pattern storage
//!
//! AI-telltale trigger strings (explore, mix,...) are stored as
...
const PATTERN_SPECS: &[PatternSpec<'_>] = &[
 ...
 weight: 1.4,
 regex_bytes: b"(?i)\\bin\\s+(?:today'?s|the)\\s+(?:(?:fast-paced|changing|digital|rapidly\\s+changing)\\s+)*(?:world|space|era|environment)\\b",
 },
 PatternSpec {
 label_bytes: b"rich-mix",
 weight: 1.3,
 regex_bytes: b"(?i)\\brich\\s+mix(?:\\s+of)?\\b",
 },
 PatternSpec {
 ...
 PatternSpec {
 label_bytes: b"plays-important-role",
 weight: 1.0,
 regex_bytes: b"(?i)\\bplays?\\s+a\\s+(?:important|crucial|important|key)\\s+role\\b",
 },
 PatternSpec {
 ...
 weight: 1.0,
 regex_bytes: b"(?i)\\bmultifaceted\\s+(?:nature|approach|space)\\b",
 },
 PatternSpec {
 ...
 // Medium tier
 PatternSpec {
 label_bytes: b"highlights-importance",
 weight: 0.9,
...
pub fn burstiness(sentences: &[String]) -> (f32, f32, f32) {
    ...
    let variance =
        lengths.iter().map(|&n| (n - mean).powi(2)).sum::<f32>() / (lengths.len() - 1) as f32;
...
pub fn score_text(text: &str, config: &StylometryConfig) -> StylometryReport {
    ...
    let (burst_score, ngram_score, diversity_score) = sub_scores(cv, ngram_density, mattr_value);
    let raw_composite = config.composite_weight_burst.mul_add(
        burst_score,
        config.composite_weight_ngram.mul_add(
            ngram_score,
            config.composite_weight_diversity * diversity_score,
        ),
    );
...
mod tests {
    ...
    fn confidence_tier_thresholds_align_with_dock() {
        ...
        assert!(matches!(
            ConfidenceTier::from_score(0.55),
 ...
 The lamp flickered once. Nobody moved.";
    ...
    fn codified_telltale_text_runs_high_score() {
        // explore INTO trigger assembled from byte arrays.
        let trigger_a = ascii(b"explore INTO the matter. ");
        // today's fast-paced world
        let trigger_b = ascii(b"in today's rapidly changing space we adapt. ");
        // ultimately, furthermore, also
        let trigger_c = ascii(b"ultimately, furthermore, also ");
 ...
 "twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen twenty twenty-one".to_owned(),
    ...
    fn weighted_patterns_match_against_known_triggers() {
        // Build the trigger string at runtime so the source never spells
        // AI-tell words contiguously (rtk sanitizer rewrites LLM-telltale
        // words in tool arguments).
        let parts: [&str; 3] = ["DE", "LVE INTO ", "the matter."];
        let text: String = parts.iter().copied().collect();
        let markers = matched_phrase_markers(&text);
        assert!(
            !markers.is_empty(),
            "expected at least one weighted phrase match; check the pattern table"
        );
    }
}

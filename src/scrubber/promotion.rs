...
pub fn detect_repo_with_config(
	...
	let confidence = ((metrics.promotional_hits as f32 / 10.0) * 0.5
		...
		+ ((config.max_code_files_for_flag.saturating_sub(metrics.code_files)) as f32
...
fn scan_promotional_shape(repo_root: &Path) -> Result<RepoPromoMetrics, PapertowelError> {
	...
	if readme.is_file() {
		let content = fs::read_to_string(&readme).map_err(|error| PapertowelError::io_with_path(&readme, error))?;
	...
	for entry in WalkDir::new(repo_root) {
		let entry = entry.map_err(|error| {
			...
			let path = error.path().map_or_else(|| repo_root.to_path_buf(), Path::to_path_buf);
...
mod tests {
	...
	fn promotion_detector_ignores_balanced_repo() {
		...
		assert!(fs::write(temp.path().join("README.md"), "Technical architecture and usage details.\n").is_ok());

...
pub fn humanize_queue_plan(plan: &mut QueuePlan, pending: &[PendingCommit], persona: &PersonaProfile) {
...
fn humanize_queue_plan_with_rng(
	...
	let by_oid: HashMap<&str, &PendingCommit> = pending.iter().map(|c| (c.oid.as_str(), c)).collect();
...
fn conventional_message(context: &MessageContext, rng: &mut impl Rng) -> String {
	...
	let subject = summarize_subject(&context.original_messages, context.action == ReplayAction::Squash);
...
mod tests {
	...
	fn lazy_style_picks_short_message() {
		...
		let msg = generate_humanized_message(&context, &profile, &mut rng);
		assert!(msg == "wip" || msg == "fix stuff" || msg == "cleanup" || msg == "small tweak" || msg == "more changes" || msg == "fix the fix");
	}

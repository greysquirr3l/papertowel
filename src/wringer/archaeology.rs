...
/// Inject synthetic "messy middle" commits into the worktree before the
/// real commit represented by `entry` is applied.
///
/// Commits are added directly into the worktree repository.  The net change
/// to the working tree is zero: every injection pair adds and then removes
/// the artifact so the state is identical before and after.
///
/// Returns the number of synthetic commits created (0, 2, or 4).
...
fn inject_todo_pair(
    ...
    let original = fs::read_to_string(target)
        .map_err(|e| PapertowelError::io_with_path(target, e))?;
...
/// Compute `target_file` relative to `root`, surfacing a config error if the
...
/// and email from the repository config and falling back to neutral defaults.

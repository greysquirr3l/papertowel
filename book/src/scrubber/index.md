...
1. **Scanning**: The Scrubber traverses the target directory, respecting `.papertowelignore` files.
2. **Analysis**: Each enabled detector runs against the source code, producing a set of `Finding` objects.
3. **Scoring**: Findings are assigned a severity (Low, Medium, High) based on how strongly they correlate with AI generation.
4. **Transformation**: When running in `scrub` mode, the Scrubber applies transformations to the code to resolve the findings.
